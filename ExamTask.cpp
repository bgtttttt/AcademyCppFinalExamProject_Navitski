...
int findBestMarkNumber(int* marks, int size) {
    ...
    for (int i = 0; i < size; ++i) {
        if (marks[i] == 4) { countFours++; }
        if (marks[i] < 2 || marks[i] > 5) {
            //cout << -1 << "\n";
            return -1;
        }
    }
    if (countFours == size) { return 0; }
    int maxFives = 0;
    for (int i = 0; i < size - 6; ++i) {
        ...
        for (int j = i; j < i + 7; ++j) {
            ...
            if (marks[j] == 5) { ++countFives; };
        }
        if (valid) { maxFives = maxFives > countFives ? maxFives : countFives; };
    }
    //cout << (maxFives > 0 ? maxFives : -1) << "\n";
